...
def line_fit(binary_warped, histogram):
    """
...
def run(img_path):
    ...
    vis_combined = np.hstack((cv2.cvtColor(combined_warped*255, cv2.COLOR_GRAY2BGR), vis_hist_combined))
    if args.vis_mode:
        imshow("vis_grad", vis_grad)
        imshow("vis_color", vis_color)
        imshow("vis_combined", vis_combined)
        
    warped_fit = line_fit(combined_warped, combined_hist)
