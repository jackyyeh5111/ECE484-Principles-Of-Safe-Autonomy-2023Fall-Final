...
def line_fit_right(binary_warped, histogram, raw_img_warped):
    """
    ...
    for leftbase in range(margin, width-margin-side_xdist, sliding_offset):
        ...
        if total_num_pixels == 0:
            continue
        even_ratio = float(left_num_pixels) / total_num_pixels * \
            float(right_num_pixels) / total_num_pixels
    ...
    leftx_base = np.argmax(
        histogram[best_left_base_x-margin:best_left_base_x+margin]) + best_left_base_x-margin
    rightx_base = np.argmax(
        histogram[best_right_base_x-margin:best_right_base_x+margin]) + best_right_base_x-margin
    ...
    def correct(currentx, y, func):

        grads = sobel_x.copy()
        grads[(binary_warped == 0)] = 0
        # idx = np.where(grads[y, currentx-margin:currentx+margin])[0] # sometimes cannot find point
    ...
    for i in range(nwindows):
        ...
        rightx_current = correct(rightx_current, y, np.argmin)
        if rightx_current - margin < 0:
            print("Lane reaches the boundary.")
            break
        lane_pt = (rightx_current, y)
...
def line_fit(binary_warped, histogram, raw_img_warped):
    ...
    for i in range(nwindows):
        ...
        try:
            leftx_current = correct(
                leftx_current, win_top, win_bottom, np.argmax)
            rightx_current = correct(
                rightx_current, win_top, win_bottom, np.argmin)
        except:
            print("Lane reaches the boundary.")
            break
        lane_pt = ((leftx_current + rightx_current) //
                   2, (win_top + win_bottom) // 2)
        lane_pts.append(lane_pt)
